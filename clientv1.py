...
class Client:
    def __init__(self, host, port=PORT,role="player",on_message=None):  # hostは必須引数に変更
        ...
        for attempt in range(MAX_CONNECT_RETRIES):
            try:
                ...
                info = {"client_type": "player"}  # or "player"
                self.socket.sendall(json.dumps(info).encode())
                connected = True
                break
        ...
        if not connected:
         ...
         # クライアントの役割をサーバーに送信
        self.send(json.dumps({"client_type": self.role}))
...
class ClientGUI:
    def __init__(self, root,role, host, port): # hostとportを受け取る
        self.root = root
        self.role=role
        self.is_watcher = (role == "watcher")
    ...
    def connect_and_setup_game(self):
        ...
        if not self.is_watcher:
            if not self.set_player_color(): # 色設定が失敗したら終了
                self.info_label.config(text="プレイヤー色の設定に失敗しました。")
                return
            self.info_label.config(text=f"あなたの色: {self.player_color} - 対戦相手を待っています...")
        else:
            self.info_label.config(text="観戦モードで接続しました")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--role",
        choices=["player", "watcher"],
        default="player",
        help="接続する役割（player または watcher）"
    )
    ...
    root = tk.Tk()
    gui = ClientGUI(root,args.role, args.server, args.port)
